...
try {
  ...
  // Magento carts
  await client.query(`
    CREATE TABLE IF NOT EXISTS magento_carts (
      id serial PRIMARY KEY,
  ...
  // Magento sync log
  await client.query(`
    CREATE TABLE IF NOT EXISTS magento_sync_log (
      id serial PRIMARY KEY,
  ...
  // Magento config
  await client.query(`
    CREATE TABLE IF NOT EXISTS magento_config (
      id serial PRIMARY KEY,
  ...
  await client.query(`
    INSERT INTO alert_config (is_enabled)
    SELECT true WHERE NOT EXISTS (SELECT 1 FROM alert_config LIMIT 1)
  `);
  ...
  // Handle upgrades from older schemas
  ...
  const { rows: roleCol } = await client.query(
