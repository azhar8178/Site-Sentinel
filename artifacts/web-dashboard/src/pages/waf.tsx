...
import { useGetServerMetrics, useGetServerWafEvents, useListServers } from "@workspace/api-client-react";
import { ShieldAlert, ShieldCheck, CircleAlert, CircleCheck, CircleX, Activity } from "lucide-react";
...
export default function Waf() {
  ...
  const { data: metrics, isLoading: metricsLoading } = useGetServerMetrics(selectedServerId ?? 0, { hours: 1 }, {
    query: { enabled: selectedServerId !== null, refetchInterval: 30000 },
  });
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-2">Web ACL health, traffic decisions, and redacted CloudWatch events.</p>
        </div>
              ...
              {metricsLoading ? (
                ...
                <>
                  ...
                  {(latestWaf.topRules?.length ?? 0) > 0 && (
                    <div className="mt-5">
                      <p className="text-sm font-semibold mb-2">Top rules in the last hour</p>
                      <div className="space-y-2">
                        {latestWaf.topRules.slice(0, 10).map((rule: any) => (
                          <div key={rule.rule} className="flex items-center justify-between text-sm">
                            <span className="truncate mr-3">{rule.rule}</span>
                            <Badge variant="secondary">{Number(rule.hits).toLocaleString()}</Badge>
                          </div>
                        ))}
                      </div>
                    </div>
